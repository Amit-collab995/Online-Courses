  ...
  return (
    <div className='flex min-h-screen p-4 bg-gray-100'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-center sm:items-center mb-6 gap-3'>
        ...
        <button className='px-4 py-2 cursor-pointer bg-black text-white rounded-2xl font-bold' onClick={()=> navigate("/createcourse")}>Create Course</button>
      </div>

      {/* for large screen */}
      <div className='hidden md:block bg-white rounded-xl shadow p-4 overflow-x-auto'>
        <table className='min-w-full text-sm'>
          <thead className='border-b bg-gray-50 class'>
            <tr>
          ...
          <tbody >
            ...
            <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
              ...
              {course.price ? <td className='px-4 py-3 '>₹ : {course?.price}</td> : <td className='px-4 py-3 '>₹ : N/A</td>}
              ...
              <td className='px-4 py-3  '>
                <FaEdit className='text-gray-600 hover:text-blue-600 text-xl  cursor-pointer'  onClick={() => navigate(`/editcourse/${course._id}`)}/>
              </td>
            </tr>
             ))}
          </tbody>
      ...
      <div className='md:hidden space-y-4'>
        ...
        <div key={index} className="bg-white rounded-lg shadow p-4 flex flex-col gap-3">
            <div key={index} className='flex gap-4  items-center'>
              {course.thumbnail ? <img src={course?.thumbnail} className='w-16 h-16 rounded-md object-contain' alt="" /> : <img src={img} className='w-16 h-16 rounded-md object-contain' alt="" />}
              <div className='flex-1'>
                <h2 className='font-medium text-sm '>{course?.title}</h2>
                {course.price ? <p className='text-gray-600 text-sm mt-1 '>RS: {course?.price}</p> : <p className='text-gray-600 text-sm mt-1 '>RS: N/A</p>}
              </div>
              <FaEdit className='text-gray-600 hover:text-blue-600 text-xl  cursor-pointer' onClick={() => navigate(`/editcourse/${course._id}`)}/>

            </div>

            <span className={`px-3 py-1 rounded-full text-xs w-fit ${course?.isPublished ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600 "} `}>{course?.isPublished ? "Published" : "Draft"}</span>
        </div>
         ))}
        <p className='text-center text-sm  mt-4 text-gray-400'>A list of your recent courses.</p>
      </div>
