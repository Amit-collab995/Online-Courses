...
const MyEnrolledCourses = () => {
    ...
    <div className='min-h-screen w-full px-4 py-9 bg-gray-50'>
        <FaArrowLeftLong className='absolute top-[3%] left-[5%] w-[22px] h-[22px] cursor-pointer  ' onClick={()=>navigate("/")} />

        <h1 className='text-3xl  text-center font-bold text-gray-800 mb-6'>My Enrolled Courses</h1>

        {
            userData?.enrollCourses?.length === 0 ? (
                    ...
                    {userData?.enrollCourses?.map((course,index)=>(
                        <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
                            ...
                            <div className='p-4'>
                                ...
                                <p>{course?.level}</p>
                                <h1 className='px-[10px] text-center py-[10px] bg-black border-2 border-black text-white rounded-[10px] text-[15px] font-light cursor-pointer mt-[10px] hover:bg-gray-600 ' onClick={()=>navigate(`/viewlecture/${course._id}`)} > Watch Now</h1>
                            </div>
                        </div>
                    ))}
                </div>
